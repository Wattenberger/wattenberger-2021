<script>
	...
	import { interpolate, range, pointer, forceCollide, forceX, forceY, pie, arc } from 'd3';
	...
	const letters = 'PIE'.split('');
	export let data = [
		{ value: 10, name: 'A' },
		{ value: 3, name: 'B' },
		{ value: 1.5, name: 'C' }
	];
	export let valueAccessor = (d) => d.value;
	...
	$: slices = pie().value(valueAccessor)(data);
	$: arcs = slices.map((d) => arc()({ ...d, innerRadius: 0, outerRadius: 100 }));
	const getPositionFromAngle = (angle, distance) => ({
		x: Math.cos(angle) * distance,
		y: Math.sin(angle) * distance
	});
	$: arcShapes = slices.map((d) => {
		const radius = 200;
		const padding = 0.1;
		const anglePerQuadrant = Math.PI / 2;
		const numberOfQuadrants = Math.max((d.endAngle - d.startAngle) / anglePerQuadrant, 1);
		const quadrantArcs = [];

		for (let i = 0; i < numberOfQuadrants; i++) {
			const startAngle = d.startAngle + padding + i * anglePerQuadrant;
			const endAngle = Math.min(d.endAngle - padding, startAngle + anglePerQuadrant);
			const startPosition = getPositionFromAngle(startAngle, radius);
			const endPosition = getPositionFromAngle(endAngle, radius);
			const midPosition = getPositionFromAngle(
				(startAngle + endAngle) / 2,
				radius * (1 + (endAngle - startAngle) * 0.23)
			);
			// {
		...
		const position = getPositionFromAngle(midAngle, 20);
		return {
			midAngle,
			position,
			path: [
				...
				// { x: 0, y: 0, z: height },
				// ...quadrantArcs.map((d) =>
				// 	d.arc ? { arc: d.arc.map((d) => ({ ...d, z: height })) } : { ...d, z: height }
				// ),
				// { x: 0, y: 0, z: height },
			]
		};
	...
	const ordinalColors = ['#8789E0', '#F2E9B0', '#B3EBF3', '#D8A5A3', '#454153'];
	...
	const onTick = () => {
		...
		$rotations = [...$rotations].map((d) => ({ ...d, y: d.y + 0.1 }));
		// if (doRotate) {
...
>
	...
	>
		...
		{#each $sparklePositions as { x, y, z }, i}
			...
			/>
		{/each}
		<Shape type="Anchor" rotate={$rotations[0]}>
			{#each arcShapes as { position, path }, i}
				<Shape type="Anchor" translate={position}>
					<Shape type="Shape" {path} fill={true} stroke={26} color={ordinalColors[i]} />
				</Shape>
			{/each}
		</Shape>
